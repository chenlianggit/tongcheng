<?php defined('IN_IA') or exit('Access Denied');?><style type="text/css">
	...
	.spectable th { font-time_money: bold;}
...
<div id='tboption' style='<?php  if($item['hasoption']!=1) { ?>display:none<?php  } ?>'>
	...
	<div id='specs'>
		...
		<?php  include $this->template('web/shop/spec')?>
		<?php  } } ?>
	</div>
	<table class="table">
		<tr>
...
<script language="javascript">
	...
	$("#add-spec").html("正在处理...").attr("disabled", "true").toggleClass("btn-primary");
		var url = "<?php  echo $this->createWebUrl('shop_spec')?>";
		$.ajax({
			"url": url,
	...
	$("#add-specitem-" + specid).html("正在处理...").attr("disabled", "true");
		var url = "<?php  echo$this->createWebUrl('shop_specitem')?>" + "&specid=" + specid;
		$.ajax({
			"url": url,
			success:function(data){
	...
	html += '<th class="info" style="width:130px;" ><div class=""><div class="option">库存</div><div class="input-group"><input type="text" class="form-control option_stock_all"VALUE=""/><span class="input-group-addon"><a href="javascript:;" class="fa fa-hand-o-down" title="批量设置" onclick="setCol(\'option_stock\');"></a></span></div></div></th>';
	html += '<th class="success" style="width:150px;"><div class=""><div class="option">销售价格</div><div class="input-group"><input type="text" class="form-control option_marketprice_all"VALUE=""/><span class="input-group-addon"><a href="javascript:;" class="fa fa-hand-o-down" title="批量设置" onclick="setCol(\'option_marketprice\');"></a></span></div></div></th>';
	html+='<th class="warning" style="width:150px;"><div class=""><div class="option">市场价格</div><div class="input-group"><input type="text" class="form-control option_productprice_all"VALUE=""/><span class="input-group-addon"><a href="javascript:;" class="fa fa-hand-o-down" title="批量设置" onclick="setCol(\'option_productprice\');"></a></span></div></div></th>';	
	html += '<th class="danger" style="width:150px;"><div class=""><div class="option">团购价格</div><div class="input-group"><input type="text" class="form-control option_groupprice_all"VALUE=""/><span class="input-group-addon"><a href="javascript:;" class="fa fa-hand-o-down" title="批量设置" onclick="setCol(\'option_groupprice\');"></a></span></div></div></th>';
	html+='<th class="success" style="width:150px;"><div class=""><div class="option">秒杀价格</div><div class="input-group"><input type="text" class="form-control option_time_money_all"VALUE=""/><span class="input-group-addon"><a href="javascript:;" class="fa fa-hand-o-down" title="批量设置" onclick="setCol(\'option_time_money\');"></a></span></div></div></th>';
	html+='</tr></thead>';
	...
	for(var i=0;i<newlen;i++){
		...
		var val ={ id : "",title:titles, stock : "",productprice : "",marketprice : "",groupprice : "",time_money:"" };
		if( $(".option_id_" + ids).length>0){
			val ={
				...
				groupprice : $(".option_groupprice_" + ids +":eq(0)").val(),
                time_money: $(".option_time_money_" + ids+":eq(0)").val()
			}
		}
		hh += '<td class="info">'
		...
		hh += '<td class="danger"><input name="option_groupprice_' +ids+'[]" type="text" class="form-control option_groupprice option_groupprice_' + ids +'" " value="' +(val.groupprice=='undefined'?'':val.groupprice )+'"/></td>';
		hh += '<td class="success"><input name="option_time_money_' + ids +'[]" type="text" class="form-control option_time_money option_time_money_' + ids +'" " value="' +(val.time_money=='undefined'?'':val.time_money )+'"/></td>';
		hh += "</tr>";
	}
	html+=hh;
	html+="</table>";
