...
static function chathtml($chatcon,$openid){
    ...
    static function getShop_apply($pageStart,$num,$sqlwhere=''){
        ...
        $sql = " FROM " . tablename(SHOP_APPLY) . " p left join ".tablename(SHOP) ." s on p.shop_id=s.shop_id and s.uniacid=p.uniacid  left join ".tablename(CATE) ." c on s.ccate_id=c.cate_id and s.uniacid=c.uniacid left join ".
            tablename(AREA) ." a on a.area_id=s.area_id and s.uniacid=a.uniacid WHERE s.uniacid = '{$_W['uniacid']}'  {$sqlwhere} ORDER BY  s.orderby DESC ";
    ...
    static function getShop($codition,$page='',$num='',$isneedpage='1'){
        ...
        $list = pdo_fetchall("SELECT s.*, c.cate_name,a.area_name FROM " . tablename(SHOP) . " s left join ".tablename(CATE) ." c on s.ccate_id=c.cate_id and s.uniacid=c.uniacid left join ".
...
//            print_r($lastedmessagelist[$k]['con']['thumbs']);exit;
    ...
    static function getCommentMsgById($page, $num, $lat, $lng){
        ...
        $condition = '';
        if(intval($id) > 0){
            $condition = ' and i.mid = '.$id;
        }
        $commentMsg=pdo_fetchall('SELECT i.*,count(c.info_id) as num FROM ' . tablename(INFO) . "i left join  " . tablename(INFO_COMMENT) . " c on i.id=c.info_id  WHERE i.weid = {$_W['uniacid']} {$condition}  GROUP BY i.id ORDER BY  num DESC LIMIT  $page,$num");
        foreach ($commentMsg as $k => $v) {
            if($lat && $lng && $v['lat'] && $v['lng']){
                $commentMsg[$k]['distance'] = util::getDistance($v['lat'], $v['lng'], $lat, $lng);
            }
    ...
    static function getHotMsgById($page, $num, $lat, $lng){
        ...
        $hotmessagelist = pdo_fetchall('SELECT * FROM ' . tablename(INFO) . " WHERE weid = {$_W['uniacid']} {$condition} AND isneedpay = 1 AND haspay > 0 AND status = 1 AND (isneedpay=0 or (isneedpay=1 and haspay=1))  ORDER BY views DESC LIMIT $page,$num");
        foreach ($hotmessagelist as $k => $v) {
            if($lat && $lng && $v['lat'] && $v['lng']){
                $hotmessagelist[$k]['distance'] = util::getDistance($v['lat'], $v['lng'], $lat, $lng);
            }
    ...
    static function getNearMsgById($page, $num, $lat, $lng){
        ...
        $nearmessagelist = pdo_fetchall("SELECT * ,ROUND(6378.138*2*ASIN(SQRT(POW(SIN(( {$lat} *PI()/180-lat*PI()/180)/2),2)+COS( {$lat} *PI()/180)*COS(lat*PI()/180)*POW(SIN(( {$lng} *PI()/180-lng*PI()/180)/2),2)))*1000) AS distance FROM " . tablename(INFO) . " WHERE weid = {$_W['uniacid']} {$condition} AND status = 1  ORDER BY distance ASC LIMIT $page,$num");
        foreach ($nearmessagelist as $k => $v) {
            $nearmessagelist[$k]['distance'] = round($nearmessagelist[$k]['distance']/1000,2);
            $module = pdo_fetch('SELECT name FROM ' . tablename(CHANNEL) . " WHERE weid = {$_W['uniacid']} AND id = {$v['mid']}");
            $nearmessagelist[$k]['con'] = unserialize($v['content']);
    ...
    static function getNewMsgById($page, $num, $lat, $lng){
        ...
        $lastedmessagelist = pdo_fetchall('SELECT * FROM ' . tablename(INFO) . " WHERE weid = {$_W['uniacid']} {$condition} AND status = 1 AND (isneedpay=0 or (isneedpay=1 and haspay=1)) ORDER BY freshtime DESC LIMIT $page,$num");
        foreach ($lastedmessagelist as $k => $v) {
            $lastedmessagelist[$k]['distance'] = util::getDistance($v['lat'], $v['lng'], $lat, $lng);
            $module = pdo_fetch('SELECT name FROM ' . tablename(CHANNEL) . " WHERE weid = {$_W['uniacid']} AND id = {$v['mid']}");
            $lastedmessagelist[$k]['con'] = unserialize($v['content']);
    ...
    static function getMsgById($id,$page,$num,$city='',$lat='',$lng='',$where='',$fid=0){
        ...
        foreach ($messagelist as $k => $v) {
            ...
            $messagelist[$k]['con'] = unserialize($v['content']);
    ...
    {
        global $_W;
        $word=pdo_fetch("SELECT * FROM " . tablename(WORD) . " WHERE uniacid = {$_W['uniacid']} and type={$type} limit 1 ");
        return $word;
    }
